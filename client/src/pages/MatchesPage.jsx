  ...
  return (
    <div style={{ animation: 'fadeInUp 0.4s ease' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: '800', fontFamily: 'Outfit,sans-serif', color: 'var(--text-primary)', marginBottom: '4px' }}>My Matches 🤝</h1>
        <p style={{ color: 'var(--text-secondary)', fontSize: '14px' }}>Track your matched resource requests and donations</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', flexWrap: 'wrap' }}>
        {['all', 'pending', 'accepted', 'completed'].map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{ padding: '8px 16px', border: `1px solid ${filter === f ? '#6366f1' : 'rgba(99,102,241,0.2)'}`, borderRadius: '9999px', background: filter === f ? 'rgba(99,102,241,0.2)' : 'transparent', color: filter === f ? 'var(--text-accent)' : 'var(--text-secondary)', cursor: 'pointer', fontSize: '13px', fontWeight: '600', fontFamily: 'Inter,sans-serif', textTransform: 'capitalize', transition: 'all 0.2s' }}>
      ...
      {loading && matches.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          {items.map(match => {
            ...
            return (
              <div key={match.id} className="card" style={{ padding: '20px', cursor: 'pointer', border: isOpen ? '1px solid var(--border-hover)' : '1px solid var(--border)', opacity: isUpdating ? 0.7 : 1 }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                  ...
                  <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: `conic-gradient(#6366f1 ${(match.matchScore || 90) * 3.6}deg, rgba(99,102,241,0.1) 0deg)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, position: 'relative' }}>
                    <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'var(--bg-card)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: '800', color: 'var(--text-accent)', fontFamily: 'Outfit,sans-serif' }}>
                      {match.matchScore || 90}%
                    </div>
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', flexWrap: 'wrap' }}>
                      <h3 style={{ fontSize: '15px', fontWeight: '700', color: 'var(--text-primary)', fontFamily: 'Outfit,sans-serif', margin: 0 }}>{match.resource}</h3>
                    ...
                    <p style={{ fontSize: '13px', color: 'var(--text-secondary)', margin: 0 }}>
                      {match.category} · By <span style={{ color: 'var(--text-accent)' }}>{match.donor || 'Community Member'}</span> · {match.date || 'Recently'}
                    </p>
                  </div>
                ...
                {isOpen && (
                  <div style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid var(--border)', animation: 'fadeIn 0.2s ease' }}>
                    <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '16px', lineHeight: '1.6' }}>{match.desc || 'A suitable match found for your resource requirements. The donor is ready to proceed with the handover process.'}</p>
      ...
      {!loading && items.length === 0 && (
        <div style={{ textAlign: 'center', padding: '60px 20px' }}>
          ...
          <h3 style={{ fontFamily: 'Outfit,sans-serif', color: 'var(--text-primary)', fontSize: '20px', marginBottom: '12px' }}>No matches yet</h3>
          <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '24px' }}>Start by browsing resources or posting a request to find a match.</p>
