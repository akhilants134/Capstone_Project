...
export default function MessagesPage({ navigate }) {
  ...
  if (!active && !loading) return (
    <div style={{ textAlign: 'center', padding: '100px' }}>
      ...
      <h2 style={{ color: 'var(--text-primary)' }}>No conversations yet</h2>
      <p style={{ color: 'var(--text-secondary)' }}>Start by browsing resources and contacting donors.</p>
  ...
  return (
    <div style={{ animation: 'fadeInUp 0.4s ease' }}>
      <div style={{ marginBottom: '20px' }}>
        <h1 style={{ fontSize: '24px', fontWeight: '800', fontFamily: 'Outfit,sans-serif', color: 'var(--text-primary)', marginBottom: '4px' }}>Messages 💬</h1>
        <p style={{ color: 'var(--text-secondary)', fontSize: '14px' }}>Communicate with donors and clients</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '20px', height: 'calc(100vh - 200px)', minHeight: '600px' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            {convos.map(c => (
              <div key={c.id} onClick={() => setActive(c)}
                style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', cursor: 'pointer', background: active?.id === c.id ? 'rgba(99,102,241,0.1)' : 'transparent', borderLeft: active?.id === c.id ? '4px solid var(--primary)' : '4px solid transparent', transition: 'all 0.2s' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                    ...
                    {c.online && <div style={{ position: 'absolute', bottom: '2px', right: '2px', width: '10px', height: '10px', borderRadius: '50%', background: '#10b981', border: '2px solid var(--bg-card)' }} />}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                      <span style={{ fontSize: '14px', fontWeight: '700', color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.name}</span>
                      <span style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>{c.time}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: '12px', color: 'var(--text-secondary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flex: 1 }}>{c.lastMsg}</span>
                      {c.unread > 0 && <span style={{ background: 'var(--primary)', color: 'white', borderRadius: '9999px', fontSize: '10px', fontWeight: '800', padding: '2px 8px', marginLeft: '8px' }}>{c.unread}</span>}
                    </div>
                  </div>
                </div>
              </div>
        ...
        {active ? (
          <div className="card" style={{ padding: '0', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '16px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '14px' }}>
              <div style={{ position: 'relative' }}>
                ...
                {active.online && <div style={{ position: 'absolute', bottom: '2px', right: '2px', width: '10px', height: '10px', borderRadius: '50%', background: '#10b981', border: '2px solid var(--bg-card)' }} />}
              </div>
              <div>
                <div style={{ fontSize: '15px', fontWeight: '800', color: 'var(--text-primary)' }}>{active.name}</div>
                <div style={{ fontSize: '11px', color: active.online ? 'var(--secondary)' : 'var(--text-secondary)' }}>{active.online ? '● Active Now' : 'Offline'}</div>
              </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {messages.map((msg, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: msg.from === 'me' ? 'flex-end' : 'flex-start' }}>
                  <div style={{ maxWidth: '70%', padding: '12px 18px', borderRadius: msg.from === 'me' ? '20px 20px 4px 20px' : '20px 20px 20px 4px', background: msg.from === 'me' ? 'var(--gradient-btn)' : 'var(--bg-input)', border: msg.from === 'me' ? 'none' : '1px solid var(--border)', boxShadow: '0 4px 15px rgba(0,0,0,0.1)' }}>
                    <p style={{ fontSize: '14px', color: msg.from === 'me' ? 'white' : 'var(--text-primary)', margin: 0, lineHeight: '1.6' }}>{msg.text}</p>
                    <span style={{ fontSize: '10px', color: msg.from === 'me' ? 'rgba(255,255,255,0.6)' : 'var(--text-secondary)', display: 'block', marginTop: '6px', textAlign: msg.from === 'me' ? 'right' : 'left' }}>{msg.time}</span>
                  </div>
                </div>
              ))}
            </div>
